...
const errorMessages = {
  ...
  serverSide: "サーブ側を選択してください",
  pressureLevel: "局面ラベルを選択してください",
  rallyLengthCategory: "ラリー長さを選択してください",
...
export function validatePointInputDraft(draft) {
  ...
  if (!isOneOf(draft?.serverSide, SERVER_SIDES)) {
    errors.push(errorMessages.serverSide);
  }

  if (!isOneOf(draft?.pressureLevel, PRESSURE_LEVELS)) {
    errors.push(errorMessages.pressureLevel);
  }

  if (!isOneOf(draft?.rallyLengthCategory, RALLY_LENGTH_CATEGORIES)) {
    errors.push(errorMessages.rallyLengthCategory);
  }
